...
captions = [Variable(torch.LongTensor([dictionary[word] if word in dictionary.keys() else dictionary['UNK'] for word in l.split()])) for l in captions]
...
with torch.no_grad():
    ...
    for cap in tqdm(captions):
        ...
        # break
    cap_mean = (cap_st_vec_total / len(captions)).cpu().detach().numpy() # torch.zeros(1, 2400) #torch.mean(cap_vec, dim=0)
...
#     novel = pickle.load(f)
#
# novel = [Variable(torch.LongTensor([dictionary[word] if word in dictionary.keys() else dictionary['UNK'] for word in l.split()])) for l in novel]
...
# # para_mean = torch.mean(para, dim=0)
